...
module.exports = function (moduleOptions) {
  ...
  // apollo
  const gqlConfig = {
    http: process.env.GRAPHQL_PROJECT_ID || this.options.env.GRAPHQL_PROJECT_ID,
    ws: process.env.GRAPHQL_SUBSCRIPTION || this.options.env.GRAPHQL_SUBSCRIPTION
  }
  this.requireModule(['@nuxtjs/apollo', {
    clientConfigs: {
      default: {
        httpEndpoint: 'https://api.graph.cool/simple/v1/' + gqlConfig.http,
        wsEndpoint: gqlConfig.ws
      }
    },
  ...
  // add font loading
  const currentFonts = 'https://fonts.googleapis.com/css?family=' + Object.keys(options.fonts).map(i => options.fonts[i]).join('|')
