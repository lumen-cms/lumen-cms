...
module.exports = async function module (moduleOptions) {
  ...
  this.addPlugin({
    src: resolve(__dirname, './templates/plugins/vueObserveVisibility.js'),
    filename: join('lumen-cms', 'plugins.vue-observe-visibility.js')
  })
  ...
  this.addPlugin({
    src: resolve(__dirname, './templates/plugins/webpsupport.js'),
    fileName: join('lumen-cms', 'plugins.supportwebp.js')
  })
  ...
  // add font loading
  const currentFonts = 'https://fonts.googleapis.com/css?family=' + Object.keys(options.fonts)
    .map(i => options.fonts[i]).filter(e => e).join('|')
  ...
  if (!this.options.disableCSS) {
    this.options.css.push({
      src: resolve(__dirname, './templates/assets/style/app.styl'),
      lang: 'styl'
    })
  }
  ...
  this.options.router.scrollBehavior = function _scrollBehavior (to, from, savedPosition) {
  ...
  this.options.build.plugins.push(new VuetifyLoaderPlugin())
  this.options.render.static = { maxAge: '7d' }
  // @TODO - make this configurable
  this.options.build.optimization.runtimeChunk = false
  ...
  this.options.build.optimization.splitChunks.name = true
  this.options.build.optimization.splitChunks.cacheGroups.commons = {
    test: /node_modules[\\/](vue|vue-loader|vue-router|vuex|vue-meta|core-js|@babel\/runtime|axios|webpack|setimmediate|timers-browserify|process|regenerator-runtime|cookie|js-cookie|is-buffer|dotprop|vue-i18n|graphql|accept-language-parser|slugify|vue-ls|eventemitter3|apollo-utilities|vue-apollo|lodash.debounce|nuxt\.js)[\\/]/,
    chunks: 'all',
    priority: 10,
    name: true
  }
