...
import { getFileAttrs } from '../components/view/helpers/imageHelper'

const getResizeValsFromString = (str) => {
  const val = str || ''
 ...
 * @returns {Object}
 */
export default (fileObj, heightResize = '', opts = {}) => {
  const widths = []
  const file = (fileObj && fileObj.file) || fileObj
  const fileWidth = file && file.width
  const fileHeight = file && file.height
  const resizeVals = (fileObj.resize && getResizeValsFromString(fileObj.resize)) || {}
  ...
  return {
    srcset: widths.length ? widths
      .map(w => `${getFileAttrs(fileObj, w + 'x' + (heightResize || fileHeight)).src} ${w}w`)
      .join(', ') : false,
