    ...
    <v-toolbar id="toolbar" slot="toolbar" dense>
      ...
      <v-tooltip top>
        <v-menu slot="activator"
          ...
          <v-list class="list-max-height">
            <v-list-tile v-for="item in iconItems" :key="item.value" @click="onSetAction('icon',item.value)">
              <v-list-tile-title :class="{'info--text':format.icon === item.value}">{{ item.text }}</v-list-tile-title>
            </v-list-tile>
          </v-list>
        </v-menu>
        <span>Icon size</span>
