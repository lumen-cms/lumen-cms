...
export default {
  ...
  render (h) {
    ...
    let childs = []
    const {height, sliderShowDelimiters, sliderAutoRotation, sliderLightDesign, sliderStyle} = this.properties
    ...
    if (sliderStyle === 'slideshow') {
      childs = items.map(item => {
        return h('v-carousel-item', {
          ...
          h('v-container', {attrs: {'fill-height': true}}, [
            h('v-layout', {atts: {row: true, wrap: true, 'align-center': true, 'fill-height': true}}, [])
          ])
        ])
      })
    } else {
