  ...
  computed: {
    ...
    activateLazy () {
      ...
      return content.fileReferences.length ||
        ...
        content.childs.find(element => (element.type === 'ListWidget') || (element.fileReferences && element.fileReferences.length) || (element.backgroundFileReferences && element.backgroundFileReferences.length))
    }
  },
  methods: {
    groupBy (collection) {
    ...
    genContainer (props, layoutType, mapped, navItems) {
      ...
      elementOptions.props.isContentElementVisible = this.isLayoutVisible
