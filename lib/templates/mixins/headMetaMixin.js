...
export default {
  ...
  methods: {
     ...
     */
    getHeadMeta ({article, languageKey, overwrites, disableCanonical}) {
      ...
      let domain = typeof CONFIG.Head.domain === 'string' ? CONFIG.Head.domain : CONFIG.Head.domain[languageKey]
      domain = domain.replace(/\/$/, '') // remove trailing slash because path always has leading slash
      ...
      const img = (media && media.previewImage && this.getImageSrc(media.previewImage))
