module.exports = function (req, res, next) {
  ...
  if (url.endsWith('/') && url.length > 1) {
    ...
    res.end()
  } else if (url.match(/favicon.ico|logo.png|robots.txt|hot-update.json|.map/g)) {
    res.statusCode = 404
    ...
    if (process.env.NODE_ENV !== 'development' &&
      ...
      // should be cached
      res.setHeader('Cache-Control', 'max-age=0,s-maxage=3600,public')
    } else {
      res.setHeader('Cache-Control', 'max-age=0,s-maxage=0,public,no-cache')
    }
    next()
  }
}
