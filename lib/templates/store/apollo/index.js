...
export const getters = {
  getPageTemplate: (state) => (section) => {
    ...
    const findTempl = state.pageTemplates.find(e => {
      return e.key === slugifiedTemplateString
    })
    if (!findTempl) {
      return null
    }
...
export const actions = {
  ...
  async setPageTemplates ({ commit }, payload) {
    return Promise.resolve(commit('SET_PAGE_TEMPLATES', payload))
  },

  async setPageProps ({ commit, state, dispatch }, payload) {
    const lang = payload.languageKey && payload.languageKey.toLowerCase()
    commit('SET_PAGE_PROPS', payload)
    if (lang && state.locale !== lang) {
      await dispatch('setLanguageKey', lang)
    }
