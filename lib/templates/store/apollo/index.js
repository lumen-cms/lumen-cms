...
export const getters = {
  getPageTemplate: (state, context) => (section) => {
    console.log('inside of vuex getter', this.app, this.$cms, context)
    const CONFIG = this.app.cms
    const findTempl = state.pageTemplates.find(e => e.key === slugifyTemplateKey(CONFIG.TEMPLATE[section], state.locale))
    if (!findTempl) {
      return ''
    }
    return findTempl
